...
// convert input to binary
    //if (){
    std::string binaryVar = std::bitset<32>(input).to_string();
    reverse(binaryVar.begin(), binaryVar.end()); // reverse the string because the compiler reads left to right
    //std::cout << binaryVar;
        if (binaryVar[b] == '1'){
            return true;
        }
...
{
    std::string binaryVar = std::bitset<32>(input).to_string();// converts to string
    if (binaryVar[0] =='1'){ // checks whether the most significant bit is a 1. if so, it is negative.
        return true;
    }
  return false;
}
